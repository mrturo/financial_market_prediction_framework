...
def test_save_and_load_json(tmp_path, sample_data):
    ...
    assert JsonManager.save(sample_data, str(file_path)) is True  # nosec
    assert file_path.exists()  # nosec
    assert JsonManager.load(str(file_path)) is not None  # nosec
...
def test_load_file_not_found(tmp_path):
    ...
    result = JsonManager.load(str(file_path))
...
def test_save_invalid_data(tmp_path):
    ...
    result = JsonManager.save(data, str(file_path))
...
def test_delete_file(tmp_path):
    ...
    """
    file_path = tmp_path / "delete_me.json"
    file_path.write_text("{}")
    result = JsonManager.delete(str(file_path))
    assert result is True  # nosec
    assert not file_path.exists()  # nosec
