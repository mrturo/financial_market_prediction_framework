...
class Gateway:
    ...
    def load() -> Dict:
        ...
        raw_data = JsonManager.load(Gateway._MARKETDATA_FILEPATH)
        ...
        invalid_symbols = Gateway._SYMBOL_REPO.get_invalid_symbols()
        enriched = {}
    ...
    def save() -> None:
        ...
        JsonManager.save(result, Gateway._MARKETDATA_FILEPATH)
        return result
    ...
    def _update_stale_symbols() -> None:
        ...
        stale = []
        invalid_symbols = set(Gateway._SYMBOL_REPO.get_invalid_symbols())
        ...
        Gateway._SYMBOL_REPO.set_invalid_symbols(invalid_symbols)
    ...
    def _update_global_schedule() -> None:
        ...
        def build_day_schedule(variants: dict) -> dict:
            ...
            for (open_str, close_str), symbols in variants.items():
                ...
                schedules.append(
                    {"open": open_str, "close": close_str, "symbols": sorted(symbols)}
                )

            min_open = min(opens).strftime("%H:%M:%S") if opens else None
            max_close = max(closes).time().strftime("%H:%M:%S") if closes else None
            all_day = (
                min_open == max_close == "00:00:00" if min_open and max_close else None
            )
