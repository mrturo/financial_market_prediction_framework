...
from prediction__predict_core import Handler, PredictionResult
...
_BACKTESTING_OUTPUT_FOLDER = _PARAMS.get("backtesting_output_folder")
...
_MODEL_PATH = _PARAMS.get("model_filepath")
_SCALER_PATH = _PARAMS.get("scaler_filepath")
...
def _load_model_and_scaler():
    ...
    model = joblib.load(_build_filename(_MODEL_PATH))
    scaler = joblib.load(_build_filename(_SCALER_PATH))
    return model, scaler
...
class BacktestSimulator:
    ...
    def save_predictions_to_csv(self):
        ...
        os.makedirs(_BACKTESTING_OUTPUT_FOLDER, exist_ok=True)

        timestamp = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
        output_path = os.path.join(
            _BACKTESTING_OUTPUT_FOLDER, f"backtest_predictions_{timestamp}.csv"
        )
