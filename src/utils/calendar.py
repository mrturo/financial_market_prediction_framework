...
def build_market_calendars(
    ...
    calendar = mcal.get_calendar(_CALENDAR_NAME)
    us_holidays = holidays.country_holidays(_HOLIDAY_COUNTRY)

    fed_events = []
    for date_str in fed_event_days:
        try:
            parsed_date = datetime.datetime.strptime(date_str, _DATE_FORMAT).date()
