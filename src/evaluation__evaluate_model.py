...
_CONF_MATRIX_PLOT_FILENAME = _PARAMS.get("conf_matrix_plot_filename")
_CORRELATIVE_SYMBOLS = _PARAMS.get("correlative_symbols")
_CUTOFF_DATE = _PARAMS.get("cutoff_date")
_EVALUATION_REPORT_FOLDER = _PARAMS.get("evaluation_report_folder")
_F1_SCORE_FIGSIZE = _PARAMS.get("f1_score_figsize")
_F1_SCORE_PLOT_FILENAME = _PARAMS.get("f1_score_plot_filename")
...
def build_calendar_context():
    ...
    _, us_holidays, fed_event_days_dt = build_market_calendars(_FED_EVENT_DAYS)
...
def generate_reports(y_true: np.ndarray, y_pred: np.ndarray):
    ...
    os.makedirs(_EVALUATION_REPORT_FOLDER, exist_ok=True)

    conf_matrix_file = os.path.join(
        _EVALUATION_REPORT_FOLDER, _CONF_MATRIX_PLOT_FILENAME
    )
    f1_score_file = os.path.join(_EVALUATION_REPORT_FOLDER, _F1_SCORE_PLOT_FILENAME)
...
def evaluate_model():
    ...
    df = load_and_process_data(_CORRELATIVE_SYMBOLS)
    if df.empty:
