...
class CoreTrainer(ABC):
    ...
    _OPTUNA_FILEPATH = _PARAMS.get("optuna_filepath")
    ...
    def save_with_timestamp(
        ...
        Logger.debug(f" * {timestamped_simple}")
        Logger.debug(f" * {timestamped_filepath}")
    ...
    def optimize_hyperparameters(
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            pipeline = self.build_pipeline(
                xgb_params,
                numerical_features=CoreTrainer._FEATURES,
                categorical_features=categorical_features or [],
            )

            if groups is not None:
                cv = GroupKFold(n_splits=CoreTrainer._N_SPLITS)
                scores = cross_val_score(
                    pipeline, x, y, groups=groups, cv=cv, scoring="accuracy"
                )
            else:
                cv = TimeSeriesSplit(n_splits=CoreTrainer._N_SPLITS)
                scores = []
        ...
        study.optimize(objective, n_trials=CoreTrainer._N_TRIALS)
        return study
    ...
    def save_artifacts(self) -> None:
        ...
        self.save_with_timestamp(self.model, CoreTrainer._MODEL_FILEPATH, "model")
        self.save_with_timestamp(self.scaler, CoreTrainer._SCALER_FILEPATH, "scaler")
        self.save_with_timestamp(
            self.study, CoreTrainer._OPTUNA_FILEPATH, "optuna_study"
        )
